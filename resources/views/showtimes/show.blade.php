        ...
        <div class="col-lg-8">
            <div class="card shadow-soft p-3 p-md-4">
                ...
                <form id="seatForm" action="{{ route('bookings.store',$showtime) }}" method="POST">@csrf
                    <div class="seat-grid">
                        @foreach($seatRows as $row => $seats)<div class="seat-row"><div class="row-label">{{ $row }}</div>@foreach($seats as $seat)@php($booked=in_array($seat->id,$bookedSeatIds))<label><input class="seat-input" type="checkbox" name="seats[]" value="{{ $seat->id }}" data-label="{{ $seat->label }}" data-price="{{ (int)$showtime->base_price+(int)$seat->price_surcharge }}" @disabled($booked)><span class="seat {{ $seat->type }} {{ $booked?'booked':'' }}">{{ $seat->number }}</span></label>@endforeach</div>@endforeach
                    </div>
                ...
                <div class="d-flex flex-wrap justify-content-center gap-4 small mt-3"><span><i class="d-inline-block border rounded bg-white" style="width:20px;height:16px"></i> Ghế thường</span><span><i class="d-inline-block border rounded" style="width:20px;height:16px;background:#fff2cc"></i> Ghế VIP (+30.000₫)</span><span><i class="d-inline-block rounded bg-danger" style="width:20px;height:16px"></i> Đang chọn</span><span><i class="d-inline-block rounded bg-secondary-subtle" style="width:20px;height:16px"></i> Đã đặt</span></div>
            </div>
        </div>
        <div class="col-lg-4">
